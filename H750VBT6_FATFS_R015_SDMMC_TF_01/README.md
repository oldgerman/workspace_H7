...
> *覆盖写入已有文件：表示已有写入 100MB 左右的文件，当前将文件指针移到地址0开始覆盖写入，这回使用前一次写入文件的 CLMT，这样 f_lseek() 的执行会更快
...
在本工程的直方图测试中，FATFS + TF卡的实时性确不太不行

为啥摄像机还可以用TF卡实时录制高清视频呢？

例如小米无人机 4K 的云台摄像机，使用 TF 卡存储实时录制的 4K 视频，我有幸拆解得知其主要器件：
...
>
> 备注：那么4K实时录像下，TF 卡平均写入速度得 6.83MB/s 左右，约54M码率
...
层缓冲区肯定越多越好，但多少个层缓冲区才可以将这个因素的影响减少到几乎没有，需要接下来的测试决定
