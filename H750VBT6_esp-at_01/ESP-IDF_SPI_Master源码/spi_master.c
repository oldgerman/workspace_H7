...
esp_err_t SPI_MASTER_ISR_ATTR spi_device_polling_start(spi_device_handle_t handle, spi_transaction_t *trans_desc, TickType_t ticks_to_wait)
{
	...
	//检查总线占用，通过CS引脚判断
    /* 如果 device_acquiring_lock 设置为 handle，这意味着用户已经通过函数
     *  `spi_device_acquire_bus()` 获得了总线。 那样的话，我们就不需要再拿锁了。 */
    ...
    if (host->device_acquiring_lock != handle) {
        ...
        if ((trans_desc->flags & SPI_TRANS_CS_KEEP_ACTIVE) != 0) {
            ...
            ret = spi_bus_lock_acquire_start(handle->dev_lock, ticks_to_wait);	//获取锁，存在最大等待时间
        }
    } else {
	...
	// 检查DMA初始化，根据handle->host的要求，选择性初始化传输缓冲区的DMA
