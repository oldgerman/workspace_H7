...
int BidirectionalPacketBasedChannel::process_packet(uint8_t* buffer, size_t length)
{
    ...
    if (packageCount & 0x8000) // BULK Packet
    {
        ...
        if (needResponse)
        {
            ...
            uint32_t n = requestedLength / CDC_DATA_FS_MAX_PACKET_SIZE;
            ...
            while (n--)
            {
                ...
                addressOffset += CDC_DATA_FS_MAX_PACKET_SIZE;
            }

            uint32_t lastPacketLength = requestedLength % CDC_DATA_FS_MAX_PACKET_SIZE;
        ...
        } else
        {
            ...
            OnBulkPacket(buffer, length,
                         addressOffsetCount * (CDC_DATA_FS_MAX_PACKET_SIZE-2),
                         0);
        }
    } else // RPC Packet
    {
