...
static xQueueHandle msg_queue; // message queue used for communicating read/write start
...
static void spi_trans_control_task(void* argument)
{
	int8_t ret;
    ...
    while (1) {
        ...
        if (recv_opt.direct == SPI_WRITE) {
            ...
            if (send_len != plan_send_len) {
                ESP_LOGE(TAG, "Read len expect %ld, but actual read %ld", plan_send_len, send_len);
                break;
            }
