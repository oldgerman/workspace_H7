...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  // (((uint32_t)size << 16) | Tx_Offset); size左移16位刚好是寄存器bit31:16, FIFO深度以32bit为单位。
  // HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 0, 0x40);就给0端点设置了0x40 * 4 字节大小的缓冲区

  HAL_PCDEx_SetRxFiFo(&hpcd_USB_OTG_FS, 0x80);      //  0x80 = 128,  128*4 = 512byte,  ALL OUT EP Buffer 共用接收FIFO
  HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 0, 0x20);   //  0x20 = 32,   32*4  = 128byte,  EP0 IN 端点0x80用于枚举
  HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 1, 0x320);  //  0x320 = 800, 800*4 = 3200byte, CDC IN 端点0x81用于BULK IN
  HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 2, 0x40);   //  0x40 = 64,   64*4  = 256byte,  CDC IN 端点0x82用于BULK IN
                                                    //  H750 USB-HS2 4KB FIFO 剩余: 4096 - 512 - 128 - 3200 -256 = 0byte
