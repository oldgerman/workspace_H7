...
void DemoRAM(uint8_t cmd)
{
	...
	switch (cmd)
	{
		...
		case '1':
			...
			DTCM_Addr1 = osRtxMemoryAlloc(AppMallocDTCM, 64, 0);
			DTCMUsed = MemHeadPtr(AppMallocDTCM);
			printf("DTCM总大小 = %d字节，申请大小 = 0064字节，当前共使用大小 = %d字节\r\n",
									                   DTCMUsed->size, DTCMUsed->used);

			/* 从DTCM申请6111字节空间，使用指针变量DTCM_Addr2操作这些空间时不要超过6111字节大小 */
			DTCM_Addr2 = osRtxMemoryAlloc(AppMallocDTCM, 6111, 0);
			DTCMUsed = MemHeadPtr(AppMallocDTCM);
		...
		case '2':
			...
			/* 释放从DTCM申请的64字节空间 */
			osRtxMemoryFree(AppMallocDTCM, DTCM_Addr1);
			DTCMUsed = MemHeadPtr(AppMallocDTCM);
			printf("释放DTCM动态内存区申请的0064字节，当前共使用大小 = %d字节\r\n", DTCMUsed->used);

			/* 释放从DTCM申请的6111字节空间 */
			osRtxMemoryFree(AppMallocDTCM, DTCM_Addr2);
			DTCMUsed = MemHeadPtr(AppMallocDTCM);
		...
		case '3':
			...
			printf("=========================================================\r\n");
			AXISRAM_Addr0 = osRtxMemoryAlloc(AppMallocAXISRAM, 160, 0);
			...
			AXISRAM_Addr1 = osRtxMemoryAlloc(AppMallocAXISRAM, 32, 0);
			AXISRAMUsed = MemHeadPtr(AppMallocAXISRAM);
			printf("AXI SRAM总大小 = %d字节，申请大小 = 0032字节，当前共使用大小 = %d字节\r\n",
									                   AXISRAMUsed->size, AXISRAMUsed->used);

			/* 从AXI SRAM 申请2333字节空间，使用指针变量AXISRAM_Addr2操作这些空间时不要超过2333字节大小 */
			AXISRAM_Addr2 = osRtxMemoryAlloc(AppMallocAXISRAM, 2333, 0);
			AXISRAMUsed = MemHeadPtr(AppMallocAXISRAM);
		...
		case '4':
			...
			osRtxMemoryFree(AppMallocAXISRAM, AXISRAM_Addr0);
			AXISRAMUsed = MemHeadPtr(AppMallocAXISRAM);
			printf("释放AXI SRAM动态内存区申请的0160字节，当前共使用大小 = %d字节\r\n", AXISRAMUsed->used);

			/* 释放从AXI SRAM申请的32字节空间 */
			osRtxMemoryFree(AppMallocAXISRAM, AXISRAM_Addr1);
			AXISRAMUsed = MemHeadPtr(AppMallocAXISRAM);
			printf("释放AXI SRAM动态内存区申请的0032字节，当前共使用大小 = %d字节\r\n", AXISRAMUsed->used);

			/* 释放从AXI SRAM申请的2333字节空间 */
			osRtxMemoryFree(AppMallocAXISRAM, AXISRAM_Addr2);
			AXISRAMUsed = MemHeadPtr(AppMallocAXISRAM);
		...
		case '5':
			...
			printf("=========================================================\r\n");
			SRAM1_Addr0 = osRtxMemoryAlloc(AppMallocSRAM1, 200, 0);
		...
		case '6':
			...
			osRtxMemoryFree(AppMallocSRAM1, SRAM1_Addr0);
			SRAM1Used = MemHeadPtr(AppMallocSRAM1);
			printf("释放D2域SRAM动态内存区申请的0200字节，当前共使用大小 = %d字节\r\n", SRAM1Used->used);

			/* 释放从D2域的SRAM申请的96字节空间 */
			osRtxMemoryFree(AppMallocSRAM1, SRAM1_Addr1);
			SRAM1Used = MemHeadPtr(AppMallocSRAM1);
			printf("释放D2域SRAM动态内存区申请的0096字节，当前共使用大小 = %d字节\r\n", SRAM1Used->used);

			/* 释放从D2域的SRAM申请的4111字节空间 */
			osRtxMemoryFree(AppMallocSRAM1, SRAM1_Addr2);
			SRAM1Used = MemHeadPtr(AppMallocSRAM1);
		...
		case '7':
			...
			printf("=========================================================\r\n");
			SRAM4_Addr0 = osRtxMemoryAlloc(AppMallocSRAM4, 300, 0);
