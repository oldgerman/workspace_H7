...
/**
 * @brief  allocates 'num' elements with size of 'nsize'
 *         分配 num 个 nsize 大小的内存块，并将所有分配的内存赋 0
 * @param  num           number of elements
 * @param  nsize         each element size
 * @retval allocated memory block or NULL in case of no memory is available.
 * */
void* osRtxMemory::calloc(size_t num, size_t nsize)
{
    if (!num || !nsize)
...
/**
 * @brief  将一个内存块重新分配大小(将已经存在的一块内存扩大)
 * @param  block           memory block to be realloced to the memory pool.
 * @param  size            size of a memory block in bytes.
 * @retval allocated memory block or NULL in case of no memory is available.
 */
void* osRtxMemory::realloc(void* block, size_t size)
{
    /*
     * https://soptq.me/2020/07/18/mem-allocator/
     * 实现逻辑：
     * 1) 若重新分配的大小 <= 原来的内存块大小，函数直接返回原来的内存块地址
     * 2) 若重新分配的大小 >  原来的内存块大小，则调用 malloc 新分配一块内存块，
     *    然后将原来的内存块里的内容复制过去，再返回新的内存块地址
     */
   if (!block || !size)    /* block and size must be valid */
       return NULL;

   // 获取当前内存块的控制块地址 Memory block header
   mem_block_t *ptr;
   ptr = MemBlockPtr(block, 0U);
   ptr--;

   if (ptr->info >= size) {  /* if the original block already has enough space for the data */
       (MemHeadPtr(mem))->used -= (ptr->info - size);
       return block;
   }
   else {
       ptr = (mem_block_t*)malloc(size); /* allocate a new space */
       if (ptr) {
           memcpy(ptr, block, ptr->info);   /* move data in block to ptr */
...
#if 0
void* osRtxMemory::aligned_calloc(size_t num, size_t nsize, size_t alignment)
{
#if 0 /* 该实现不正确，待研究 */
    if (!num || !nsize)
        return NULL;    /* num and nsize must be valid */
    ...
    if (nsize != size / num)
        return NULL;    /* multiply overflow */
    block = aligned_malloc(size, alignment);
    if (!block)
        return NULL;    /* malloc failed */
    memset(block, 0, size); /* reset all value in this block */
    return block;
...
void* osRtxMemory::aligned_realloc(void* block, size_t size, size_t alignment)
{
#if 0 /* 该实现不正确，待研究 */
   if (!block || !size)    /* block and size must be valid */
       return NULL;
   ...
   if (ptr->info >= size) {  /* if the original block already has enough space for the data */
       ...
       ptr = (mem_block_t*)aligned_malloc(size, alignment); /* allocate a new space */
       if (ptr) {
           memcpy(ptr, block, ptr->info);   /* move data in block to ptr */
           aligned_free(block);    /* free original block */
       }
       return ptr;
   }
#endif
