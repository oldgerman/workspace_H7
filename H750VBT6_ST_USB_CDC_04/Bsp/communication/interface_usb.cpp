 ...
 */
StreamSink *usb_stream_output_ptr = &usb_stream_cdc_output;
    ...
    for (;;)
    {
        ...
        if (sem_stat == osOK)
        {
            ...
            if (CDC_interface.data_pending)
            {
                ...
                ASCII_protocol_parse_stream(CDC_interface.rx_buf, CDC_interface.rx_len, usb_stream_cdc_output);
...
void usb_rx_process_packet(uint8_t *buf, uint32_t len , uint8_t endpoint_pair)
{
    ...
    /* CDC_interface.out_ep */
    if (endpoint_pair == CDC_interface.out_ep)
    {
        usb_iface = &CDC_interface;
    }
    /* ODrive_interface.out_ep */
    else if (endpoint_pair == BULK_interface.out_ep)
    {
        usb_iface = &BULK_interface;
    } else
    {
        return;
    }

    // 在前一个数据包被完全处理之前，不允许处理下一个 USB 数据包。
    // 因此直接写入这些变量是安全的，因为我们知道之前的处理已经完成。
    ...
    usb_iface->rx_buf = buf;			// 指向缓冲区
    usb_iface->rx_len = len;			// 缓冲区长度
    usb_iface->data_pending = true;		// 数据待处理标记为true

    osSemaphoreRelease(sem_usb_rx);		// 释放信号量
}
