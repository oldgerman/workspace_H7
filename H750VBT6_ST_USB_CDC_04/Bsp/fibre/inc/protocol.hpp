...
extern uint8_t * OnBulkPacket(const uint8_t* _buffer, size_t _bufferLength, uint32_t _packetWriteAddressOffset, uint32_t _readDataLength);
...
template<typename T>
static inline T read_le(uint8_t** buffer, size_t* length)
{
    T result;
...
public:
    ...
    virtual int process_packet(uint8_t* buffer, size_t length) = 0;
};
...
public:
    ...
    // @return: 0 on success, otherwise a non-zero error code
    virtual int process_bytes(uint8_t* buffer, size_t length, size_t* processed_bytes) = 0;
...
public:
    ...
    int process_packet(uint8_t* buffer, size_t length);
...
typedef std::function<void(void* ctx, const uint8_t* input, size_t input_length,
                           StreamSink* output)> EndpointHandler;
...
template<typename T>
bool
default_readwrite_endpoint_handler(endpoint_ref_t* value, const uint8_t* input, size_t input_length,
                                   StreamSink* output)
{
...
{
    return output->process_bytes((uint8_t*) str, strlen(str), nullptr);
}
...
* support the REF ASCII protocol.
...
{
public:
    static constexpr size_t endpoint_count =
        TMember::endpoint_count + MemberList<TMembers...>::endpoint_count;
...
template<typename TProperty, ENABLE_IF(std::is_enum<TProperty>::value) >
ProtocolProperty<std::underlying_type_t<TProperty>>
make_protocol_property(const char* name, TProperty* property,
                       void (* written_hook)(void*) = nullptr,
                       void* ctx = nullptr)
{
...
struct PropertyListFactory<>
{
    ...
    static MemberList<>
    make_property_list(std::array<const char*, sizeof...(TAllProperties)> names,
...
public:
    ...
    ProtocolFunction(const char* name, TObj &obj, TRet(TObj::*func_ptr)(TInputs...),
        ...
        input_properties_(
            PropertyListFactory<TInputs...>::template make_property_list<0>(input_names_, in_args_)),
        output_properties_(
    ...
    void register_endpoints(Endpoint** list, size_t id, size_t length)
    {
        ...
        output_properties_.register_endpoints(list, id + 1 + decltype(input_properties_)::endpoint_count,
                                              length);
    }
