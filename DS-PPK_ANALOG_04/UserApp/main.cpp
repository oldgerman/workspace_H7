...
//		bsp_adc3GetValues();
        ...
        {
			static uint32_t adc_convCallbackCnt_old = 0;
			printf("[adc_convCallbackCnt] %.lu times 1s\r\n", adc_convCallbackCnt - adc_convCallbackCnt_old);
			adc_convCallbackCnt_old = adc_convCallbackCnt;

			static uint32_t logic_convCallbackCnt_old = 0;
			printf("[logic_convCallbackCnt] %.lu times 1s\r\n", logic_convCallbackCnt - logic_convCallbackCnt_old);
			logic_convCallbackCnt_old = logic_convCallbackCnt;
	    }
		debug_IDR.ctrl = GPIOC->IDR;
...
//    cw_bat_init();
    ...
    bsp_logicInit();               /* 初始化逻辑通道，并设置VCCB=3.3V */
	bsp_adc2Init();
	bsp_adc1Init();
	bsp_adc3Init();
	bsp_auto_sw_init();
	bsp_smuEnableVoutMosfet(true); /* 打开电压输出MOS */
