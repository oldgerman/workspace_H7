...
ALIGN_32BYTES(__attribute__((section (".RAM_D2_Array"))) uint16_t adc1_data[sample_buffer_size]);
...
{
	/* 启动ADC的DMA方式传输 */
	if (HAL_ADC_Start_DMA(&hadc1, (uint32_t *)adc1_data, sample_buffer_size) != HAL_OK)
	{
	    Error_Handler();
	}
}
