...
extern uint32_t sampleTrigFreq;
/**
  * sample_buffer_size 必须>=800点且为400点的倍数，以保证每次发送至少400点
  * 0x320 * 4 / 2  usb ep1 tx fifo 大小的一半 1600KB，400点每个点4byte，每次向 USB TX FIFO 发送1600byte
  * 2000样点存在data loss情况
  * 4000样点此症状消失
  * 8000样点刷新太慢
  */
...
/* Exported variables --------------------------------------------------------*/
extern float vref;
extern pwmSet_InfoTypeDef sampleTrigPWMInfo;
extern volatile adc2_chx_values_t adc2_values;
extern uint16_t adc1_data[sample_buffer_size];
extern uint16_t adc3_data[sample_buffer_size];
extern TickType_t xFrequency_adc1Task;
extern volatile uint32_t adc_convCallbackCnt;
extern volatile uint8_t adc_convCallbackEnd;
