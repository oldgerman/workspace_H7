		...
		float pwmDutyCycle)
{
	...
	uint32_t calcClockFreq = LptimClockFreq;
	uint32_t ARR = 0;
	uint32_t PSC = 0;
	uint32_t CMP = 0;
	...
	for(uint16_t i = 0; i < 8; i++){
		PSC = twoNthPower(i);
		ARR = (float)calcClockFreq / PSC / pwmFrequency + 0.5;	// 注意对ARR进行了四舍五入
	...
	/* 计算实际pwm参数 */
	float pwmFrequency_float = (float)calcClockFreq / PSC / ARR;	// pwmFreq = clockFreq / (PSC * ARR)
	...
	PSC = (uint32_t)(twoNthPowerOfNth(PSC) << LPTIM_CFGR_PRESC_Pos) & LPTIM_CFGR_PRESC_Msk;	// 得到PSC在CFGR的3bit段值,注意得到000~111要将值twoNthPowerOfNth(PSC)减去1
