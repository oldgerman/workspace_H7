...
ADC传输中断回调必须立即响应以准确记录时间戳，所以中断优先级得设置得比5小，这样才不会被 FreeRTOS或用户程序的临界段临时屏蔽，这样如何同步中断和帧处理器任务呢？那必须用全局变量了，暂且叫它 X，在ADC中断回调函数中修改 X 为 1，那么帧处理任务得由事件驱动改为时间片轮调，由于没有信号量同步中断和帧处理器任务，帧处理任务不会立即响应的可能性很大，最好加大ADC缓冲区保证等待调度的期间一定能处理完其他高优先级的任务，拟4倍好了，2000点增大到8000点，那么100Hz 的调度频率降低为25Hz，要保证帧处理器每 40ms 必须调度 1次，每次轮调（关全局中断0~15） if 判断 X 是否为1，若为1立即改为0（打开全局中断0~15，注意需要保证该任务改为0的期间ADC中断马上不会响应）缺点是多耗费点内存，ADC 缓冲区增大为 31.25KB，时间戳缓冲区增大为 39.06KB
