...
Vofa+ 添加一个测试换挡尖峰专用的命令组，发送校准电阻命令触发换挡命令间隔10ms，连续发送10次，一次命令组发送40次
...
放大局部，将红色箭头的视为离谱尖峰，蓝色视为微弱尖峰

![](Images/换挡尖峰测试2.png)

继续放大，发现了戏剧性的一幕，每一个微弱尖峰后都有一个离谱尖峰

![](Images/换挡尖峰测试3：整整齐齐有规律的尖峰.png)

### 分析换挡尖峰出现的原因

发送命令设置校准电阻从0.1mA切换到 10mA，切换瞬间的尖峰却达到了 500mA，显然切到 10mA～ 100mA 量程 是不可能 量出 500mA 的，归其原因，应该是量程时间戳和ADC传输中断时间戳没有准确地对齐

当前代码中，ADC传输中断回调函数记录时间戳后释放信号量解除帧处理器任务的阻塞，由于在中断中调用FreeRTOS API，那么ADC中断的优先级配置为5，那么运行时就可能被 FreeRTOS内的临界段代码屏蔽了一会儿中断，导致延时进入ADC中断回调函数，那么在ADC传输中断回调函数内记录的时间戳就滞后于实际ADC缓冲区最后一个数据被填充的时刻，继而导致在帧处理器中（根据换挡和ADC传输双缓冲区时间戳）对齐量程和ADC的数据时，发生错位（上文使用命令组周期性发送换挡命令时，可见尖峰极其有规律地出现，真可谓：一步错，步步错）

只要ADC回调函数记录的时间戳滞后时间大于定时器计数器的1个steps，即10us，就会产生错位，这样就会发生1档对应的ADC测量值比如2V，错位对成2档，导致计算结果变大10倍，甚至对成3档，计算成100倍，对成4档计算成1000 倍，反之，高档向低档切换对齐错位会计算结果变为1/10，1/100，1/1000等，所以这应该就是切换瞬间上下尖峰产生的原因

不知道为啥，感觉下尖峰出现的概率比上尖峰低：

![](Images/换挡尖峰测试：下尖峰.png)

### FreeRTOS 相关知识补充

> 出处：安富莱 V6 FreeRTOS教程

**15.1 临界段**

![](Images/armfly_v6_15.1临界段.png)

**12.4 不受 FreeRTOS 管理中断的深入讨论**

![](Images/armfly_v6_12.4不受FreeRTOS管理中断的深入讨论.png)

### 拟解决方法

ADC传输中断回调必须立即响应以准确记录时间戳，所以中断优先级得设置得比5小，这样才不会被 FreeRTOS或用户程序的临界段临时屏蔽，这样如何同步中断和帧处理器任务呢？那必须用全局变量了，暂且叫它 X，在ADC中断回调函数中修改 X 为 1，那么帧处理任务得由事件驱动改为时间片轮调，由于没有信号量同步中断和帧处理器任务，帧处理任务不会立即响应的可能性很大，最好加大ADC缓冲区保证等待调度的期间一定能处理完其他高优先级的任务，拟4倍好了，2000点增大到8000点，那么100Hz 的调度频率降低为25Hz，要保证帧处理器每 40ms 必须调度 1次，每次轮调（关全局中断） if 判断 X 是否为1，若为1立即改为0（打开全局中断），注意需要保证该任务改为0的期间ADC中断马上不会响应）缺点是多耗费点内存，ADC 缓冲区增大为 31.25KB，时间戳缓冲区增大为 39.06KB
...
static void frameProcessorTask(void* argument)
{
    ...
    for(;;)
    {
        /* 保证此期间ADC中断回调不会修改X为1，导致下次调度此任务时错失一次帧处理 */
        primask = __get_PRIMASK();
        __disable_irq();
        if (X == 1)
        {
            X = 0;
            __set_PRIMASK(primask); //修改X值为0后必须立即打开全局中断
            // 处理帧数据
            ...
        }else{
            __set_PRIMASK(primask); //若X == 0，也要立即打开全局中断
        }
        vTaskDelayUntil(...);  // 25Hz调度频率
    }
}
```

但要注意，这段开关全局中断的代码执行时间要小于 1 steps，即10us，否则换挡外部中断或ADC传输中断时间戳还是会延时记录，使用时间戳对齐双缓冲区数据时还是会错位产生尖峰

注：[STM32开关全局中断的多种方式](https://blog.csdn.net/working24hours/article/details/88323241)

若不想调用__disable_irq()，那么调度频率得换成 50Hz，实际处理频率还是 25Hz，原因需要画时序图解释（渣绘）

![ADC中断使用全局变量向帧处理任务通信的问题](Images/ADC中断使用全局变量向帧处理任务通信的问题.png)
...
将帧处理器任务函数运行域编译到 RAM，时间戳双缓冲区编译到 DTCM

### 仿真出现的尖峰

仿真时切换瞬间也会有尖峰，这个应该是仪放满偏导致？硬件上的尖峰问题

### 数字滤波器

使用 DSP 库的 FIR 低通滤波器 进行实时滤波，待实现
