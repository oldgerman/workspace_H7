...
#if 0
		__HAL_DMA_SET_COUNTER(&hdma_usart1_tx, usart_tx_dma_current_len);
		/* Memory to Peripheral */  /* Configure DMA Stream source address */
		((DMA_Stream_TypeDef *)(hdma_usart1_tx.Instance))->PAR = (uint32_t)lwrb_get_linear_block_read_address(&usart_tx_rb);
...
void re_DMA1_Stream0_IRQHandler(void) {
	...
	if (__HAL_DMA_GET_IT_SOURCE(&hdma_usart1_rx, DMA_IT_HT) &&  __HAL_DMA_GET_FLAG(&hdma_usart1_rx, DMA_FLAG_HTIF0_4)){
		__HAL_DMA_CLEAR_FLAG(&hdma_usart1_rx, DMA_FLAG_HTIF0_4);
        usart_rx_check();                       /* Check for data to process */
    }
    /* Check transfer-complete interrupt */
	if (__HAL_DMA_GET_IT_SOURCE(&hdma_usart1_rx, DMA_IT_TC) &&  __HAL_DMA_GET_FLAG(&hdma_usart1_rx, DMA_FLAG_TCIF0_4)){
...
void re_DMA1_Stream1_IRQHandler(void) {
    ...
    if (__HAL_DMA_GET_IT_SOURCE(&hdma_usart1_tx, DMA_IT_TC) && __HAL_DMA_GET_FLAG(&hdma_usart1_tx, DMA_FLAG_TCIF1_5)) {
    	__HAL_DMA_CLEAR_FLAG(&hdma_usart1_tx, DMA_FLAG_TCIF1_5);             /* Clear transfer complete flag */
...
void loop(){
	    ...
	    while (1) {
//	    	uint8_t str[] = "abcdefg\r\n";
//	    	HAL_UART_Transmit(&huart1, str, 10, 100);
	        uint8_t b;
...
// https://blog.csdn.net/pingxiaozhao/article/details/122884674
//int pos = 0;
//void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
//{
//	uint32_t temp;
//	if((__HAL_UART_GET_FLAG(huart,UART_FLAG_IDLE) != RESET))	//检查空闲中断标志位
//	{
//		__HAL_UART_CLEAR_IDLEFLAG(huart);						//清除空闲中断标志
//		HAL_UART_DMAStop(huart);								//由于是RX DMA是循环模式，而中断内要处理数据，所以要暂停UART DMA
//		temp = __HAL_DMA_GET_COUNTER(&hdma_usart1_rx);			//获取RX DMA 接收字符 数量
//		if(huart->Instance==USART1)								//若为空闲中断来自 USART1
//		{
...
//        __HAL_DMA_SET_COUNTER(&hdma_usart1_tx, usart_tx_dma_current_len);
        /* Memory to Peripheral */  /* Configure DMA Stream source address */
//        ((DMA_Stream_TypeDef *)(hdma_usart1_tx.Instance))->M0AR = (uint32_t)lwrb_get_linear_block_read_address(&usart_tx_rb);
...
//    __HAL_DMA_SET_COUNTER(&hdma_usart1_rx, ARRAY_LEN(usart_rx_dma_buffer));
    /* Peripheral to Memory */  /* Configure DMA Stream source address */
//    ((DMA_Stream_TypeDef *)(hdma_usart1_rx.Instance))->M0AR = (uint32_t)usart_rx_dma_buffer;
