...
uint8_t CDC_Transmit_FS(uint8_t* Buf, uint16_t Len)
{
  ...
  // Refer to the implementation of the following two:
  // https://github.com/kauailabs/navxmxp/blob/master/stm32/Src/usbd_cdc_if.c
  // https://blog.csdn.net/u010779035/article/details/104369515

  USBD_CDC_HandleTypeDef *hcdc = (USBD_CDC_HandleTypeDef*)hUsbDeviceFS.pClassData;

  /* If the length of the transmitted data is greater than APP_TX_DATA_SIZE , send it multiple times */
  if (Len > APP_TX_DATA_SIZE)
  {
	  uint16_t offset;
	  for (offset = 0; offset < Len; offset++)
	  {
		  uint16_t todo = MIN(APP_TX_DATA_SIZE,
				  Len - offset);
		  result = CDC_Transmit_FS(Buf + offset, todo);	// recursion
  ...
  /* Wait up to 10ms for the last transfer to complete
   * The maximum speed of USB FS CDC is about 800KB/S,
   * so the transmission length within 10ms is up to 8192Byte */
  uint32_t tx_completion_wait_timestamp = HAL_GetTick();
  while(hcdc->TxState)
  {
	  if ( HAL_GetTick() - tx_completion_wait_timestamp > (uint32_t)10 ) {
		  /* Timeout */
		  return USBD_BUSY;
	  }
  }
