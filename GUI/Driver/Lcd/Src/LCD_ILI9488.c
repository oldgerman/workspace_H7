...
*		V1.1	2016-04-23 armfly  新增 ILI9488_FillCircle 桉树
*
...
#else
#define ILI9488_BASE       ((uint32_t)(0x6C000000 | 0x00000000))		// 安富莱V5开发板使用FMC_NE4：首地址 0x6C00 0000，可以管理的地址范围 0x6C00 0000 到 0x6FFF FFFF
#define ILI9488_REG		*(__IO uint16_t *)(ILI9488_BASE)
#define ILI9488_RAM		*(__IO uint16_t *)(ILI9488_BASE + (1 << (18 + 1)))	/* FSMC 16位总线模式下，FSMC_A18口线对应物理地址A19 // v5开发板接线：PD13/FSMC_A18 --- 地址 RS*/
#endif
...
void ILI9488_InitHard(void)
{
	...
	if (id == 0x548066)		//ILI9488 ID code #define ILI9488_DEVICE_CODE (0x9488u)
	{
...
static void Init_9488(void)
{
	...
	// 参数一
//	ILI9488_WriteParam(0x20);	// 0010,0000  CNT = 33 	32.17FPS	30.89FPS
	ILI9488_WriteParam(0xB0);	// 1011,0000  CNT = 15 	68.36FPS	65.65FPS
//	ILI9488_WriteParam(0xC0);	// 1101,0000  CNT = 13 	78.13FPS	75.03FPS
//	ILI9488_WriteParam(0xD0);	// 1101,0000  CNT = 11 	91.15FPS	87.53FPS
	// 参数二
	ILI9488_WriteParam(0x11);	// 0001,0001  CLOCK PER LINE = 17 clocks
//	ILI9488_WriteParam(0x10);	// 0001,0000  CLOCK PER LINE = 16 clocks

	 /* Tearing Effect Line On: Option (00h:VSYNC Interface OFF, 01h:VSYNC Interface ON) */
	//参考line:237行 https://github.com/STMicroelectronics/STM32CubeL4/blob/master/Drivers/BSP/Components/st7789h2/st7789h2.c
	//不能开Tearing ON，8080驱动情况下开了会产生刷新坐标抖动
//	ILI9488_WriteCmd(0x35);
//	ILI9488_WriteParam(0x00);
 ...
 *  等效ILI9488_SetDispWin，只是后两个参数变为坐标
...
void ILI9488_Address_Set(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
    ...
    ILI9488_WriteCmd(0x2A);	//设置行地址
	ILI9488_WriteParam(x1 >> 8);			//保留uint16_t类型的高8位，右对齐
	ILI9488_WriteParam(x1 & 0xFF); 			//保留uint16_t类型的低8位，右对齐
	ILI9488_WriteParam(x2 >> 8);			//保留uint16_t类型的高8位，右对齐
	ILI9488_WriteParam(x2 & 0xFF);			//保留uint16_t类型的低8位，右对齐

    ILI9488_WriteCmd(0x2B);	//设置页地址
    ILI9488_WriteParam(y1 >> 8);			//保留uint16_t类型的高8位，右对齐
    ILI9488_WriteParam(y1 & 0xFF);			//保留uint16_t类型的低8位，右对齐
    ILI9488_WriteParam(y2 >> 8);			//保留uint16_t类型的高8位，右对齐
    ILI9488_WriteParam(y2 & 0xFF);			//保留uint16_t类型的低8位，右对齐
}
/*
*********************************************************************************************************
...
*********************************************************************************************************
*/
void ILI9488_SetCursor(uint16_t _usX, uint16_t _usY)
{
	ILI9488_WriteCmd(0X2A); 		/* 设置X坐标 */
...
/*
*********************************************************************************************************
*	函 数 名: ILI9488_GetCursorAddress
*	功能说明: 得到光标位置在GRAM的地址（FMC总线上）,必须为退出窗口，即整个屏幕开窗才行
*	形    参:  x : X坐标; y: Y坐标
*	返 回 值: FMC总线上的地址
*********************************************************************************************************
*/
...
{
	return (uint32_t)0x60100000 + (320 * y + x) * 2;
	//LCD显存首地址 + 坐标相对于显存起始像素的偏移像素个数 乘以二 是因为16bit模式
}
/*
*********************************************************************************************************
...
*	形    参:  无
*	返 回 值: 无
*	注释: ILI9488手册P151:  5.2.3. Read Display Identification Information (04h)
*********************************************************************************************************
*/
...
void ILI9488_DrawLine(uint16_t _usX1 , uint16_t _usY1 , uint16_t _usX2 , uint16_t _usY2 , uint16_t _usColor)
{
	...
	if (_usX1 == _usX2)
	{
		if (_usY2 > _usY1)
		{
			...
			{
				ILI9488_PutPixel(_usX1, y, _usColor);
			}
		}
		else
		{
	...
	if (_usY1 == _usY2)
	{
		if (_usX2 > _usX1)
		{
			...
			{
				ILI9488_PutPixel(x, _usY1, _usColor);
			}
		}
		else
		{
...
void ILI9488_FillCircle(uint16_t _usX, uint16_t _usY, uint16_t _usRadius, uint16_t _usColor)
{
	...
	while (CurX <= CurY)
	{			
		ILI9488_DrawLine(_usX + CurX, _usY + CurY, _usX - CurX, _usY + CurY, _usColor);
		ILI9488_DrawLine(_usX + CurX, _usY - CurY, _usX - CurX, _usY - CurY, _usColor);
		ILI9488_DrawLine(_usX + CurY, _usY + CurX, _usX - CurY, _usY + CurX, _usColor);
		ILI9488_DrawLine(_usX + CurY, _usY - CurX, _usX - CurY, _usY - CurX, _usColor);  			
